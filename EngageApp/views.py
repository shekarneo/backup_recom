...
from . models import (EventDetails, EventAssociation, EventDelegates, EventDates,
                      ...
                      EventSalesPersons, EventSpeakers, EventSupportedBy, EventTestimonials, EventVisitors,AboutUs,EventExhibitor_1,
                      EventConference_1,EventVisitor_1,VirtualRegister,EventMediaPartner)
from .serializers import (EventSerializer,AssociationSerializer,DelegateSerializer,EventDateSerializer,
                          ...
                          EventSalesPersonSerializer,EventSpeakersSerializer,EventSupportedSerializer,EventTestimonialsSerializer,
                          EventVisitorSerializer,aboutUsSerializer, EventSerializerReq,EventMediaSerializer,EventAssociateSerializer)
...
class EventData(APIView):
    def post(self, request):
        ...
        speakers = EventSpeakers.objects.raw("SELECT * FROM engageapp_eventspeakers WHERE event_id = {0} ORDER BY SUBSTRING_INDEX(SUBSTRING_INDEX(vc_speaker_name, ' ', 2), ' ', -1)".format(id))
...
class DigitalPartner(APIView):
    def post(self, request):
        register_data = JSONParser().parse(request)
        id = register_data['id']
        partners = EventPartners.objects.raw(("SELECT * FROM engageapp_eventpartners WHERE  event_id= {0} AND vc_partner_type='digital'").format(id))
        event_serializer = EventPartnersSerializer(partners, many=True)
...
class AssociatePartner(APIView):
    def post(self, request):
        register_data = JSONParser().parse(request)
        id = register_data['id']
        partners = EventPartners.objects.raw("SELECT * FROM engageapp_eventpartners WHERE event_id={0} AND vc_partner_type = 'associate'".format(id))
        event_serializer = EventPartnersSerializer(partners, many=True)
...
class MediaPartner(APIView):
    def post(self, request):
        register_data = JSONParser().parse(request)
        id = register_data['id']
        partners = EventPartners.objects.raw("SELECT * FROM engageapp_eventpartners WHERE event_id={0} AND vc_partner_type = 'media partner'".format(id))
        event_serializer = EventPartnersSerializer(partners, many=True)
        return JsonResponse({'associate_partners': event_serializer.data}, safe=False)
